// Tidying up a sentence somebody typed on the floor.
//
...
//
// So there is a Tidy button. It fixes spelling, grammar and punctuation and changes nothing
// else — and the person who wrote the line sees the result in their own box before it is
// saved, with one click to put their own words back. Nothing is rewritten behind anybody.
//
...
Deno.serve(async request => {
  ...
  // Signed in, and that is the whole check. Anyone who can write a comment can tidy one —
  // this is spelling, not access to anything.
  ...
  if (!KEY) return reply({ unavailable: true, error: 'Tidy-up is not set up for this plant.' }, 200);
  ...
  if (text.length > LONGEST) {
    return reply({ error: `That is longer than ${LONGEST} characters. Tidy it a line at a time.` }, 400);
  }
  ...
  try {
    ...
    if (message.stop_reason === 'refusal') {
      return reply({ error: 'That note could not be tidied. Keep your own wording.' }, 200);
    }
    ...
    return reply({ error: (error as Error).message || 'Tidy-up failed.' }, 200);
  }
});
